...
SELECT 
    ...
    ROUND(AVG(count_per_order.oc * 1.0), 2) AS average_orders
FROM customers 
...
-- 14. Calculate the retention rate of customers (repurchase within 6 months).
...
next_orders_within_6_months AS (
    ...
    WHERE orders.order_purchase_timestamp > first_orders.first_order_date
      AND orders.order_purchase_timestamp <= DATEADD(MONTH, 6, first_order_date)
    GROUP BY customers.customer_id
)
SELECT
    ROUND(
